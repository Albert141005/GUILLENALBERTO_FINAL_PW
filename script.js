document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("emailForm").addEventListener("submit", function (event) {
        ...
        emailMessage.textContent = `Correo enviado a: ${email}`;
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    postElement.innerHTML = `<h3>${post.title}</h3><p>${post.body}</p>`; // Muestra el título y el cuerpo
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                starWarsDataDisplay.innerHTML = `
                    ...
                    <p>Masa: ${data.masa} kg</p>
                    <p>Color de cabello: ${data.hair_color}</p>
                    <p>Color de piel: ${data.skin_color}</p>
                    <p>Color de ojos: ${data.eye_color}</p>
                    <p>Año de nacimiento: ${data.año_de_nacimiento}</p>
                    <p>Género: ${data.género}</p>
                    <p>Mundo natal: <a href="${data["mundo natal"]}">Ver planeta</a></p>
                    <p>Películas: ${data.películas.map(url => `<a href="${url}">Ver película</a>`).join(', ')}</p>
                    <p>Especies: ${data.especies.map(url => `<a href="${url}">Ver especie</a>`).join(', ')}</p>
                    <p>Vehículos: ${data.vehículos.map(url => `<a href="${url}">Ver vehículo</a>`).join(', ')}</p>
                    <p>Naves espaciales: ${data["naves espaciales"].map(url => `<a href="${url}">Ver nave</a>`).join(', ')}</p>
                `;
            })
            .catch(error => {
